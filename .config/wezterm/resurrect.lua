...
M.apply_to_config = function(config)
  ...
  resurrect.state_manager.change_state_save_dir(home .. sep .. "dotfiles/.config/wezterm/saved")
  ...
  table.insert(config.keys, {
    ...
    action = wezterm.action_callback(function(win, pane)
      ...
      if not success then
        wezterm.log_error("resurrect fuzzy_load error: " .. tostring(err))
      end
    end),
  })
  ...
  -- path = the clean workspace name (id), label = formatted display string - use path to load state
  wezterm.on("smart_workspace_switcher.workspace_switcher.created", function(window, path, label)
    local workspace_state = resurrect.workspace_state

    workspace_state.restore_workspace(resurrect.state_manager.load_state(path, "workspace"), {
      window = window,
      relative = true,
      restore_text = true,
      on_pane_restore = resurrect.tab_state.default_on_pane_restore,
